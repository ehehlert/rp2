# python -m src.pipelines.seam_reports.classify_blocks.train
...
directory = 'src/pipelines/seam_reports/classify_blocks'
...
# Load the dataset
tagged_df = pd.read_csv(input_path).drop(columns=['Page', 'Confidence', 'Source'])

# Initialize the tokenizer
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')

# Tokenize the text
inputs = tokenizer(tagged_df['Text'].tolist(), return_tensors='pt', padding=True, truncation=True, max_length=512)

# Convert labels to numerical format
label_encoder = LabelEncoder()
labels = label_encoder.fit_transform(tagged_df['block_role'])
